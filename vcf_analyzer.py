...
def set_plot_style():
    """Set global plot styling for professional appearance."""
    plt.style.use('seaborn-v0_8-whitegrid')
    plt.rcParams.update({
        'font.size': 16,                    # Increased base font size
        'axes.titlesize': 20,               # Increased title size
        'axes.labelsize': 18,               # Increased label size
        'xtick.labelsize': 16,              # Increased tick label size
        'ytick.labelsize': 16,              # Increased tick label size
        'legend.fontsize': 16,              # Increased legend font size
        'legend.title_fontsize': 18,        # Increased legend title size
        'figure.titlesize': 22,             # Increased figure title size
        ...
        'axes.grid': True,
        'grid.alpha': 0.3,
        'axes.linewidth': 2,                # Increased line width
        'lines.linewidth': 3,               # Increased line width
        'lines.markersize': 12,             # Increased marker size
        'savefig.dpi': 300,
        'savefig.bbox': 'tight',
        'savefig.pad_inches': 0.2,
        'mathtext.default': 'regular',      # Make math text regular weight
        'mathtext.fontset': 'stix',         # Use STIX font for math
        'font.family': 'sans-serif',        # Use sans-serif font
        'font.sans-serif': ['Arial', 'DejaVu Sans', 'Helvetica', 'sans-serif']  # Preferred fonts
    })

def plot_metrics_vs_variants(test_metrics_list, window_stats, output_dir):
    """Plot macro and weighted metrics against number of variants for each window."""
    try:
        set_plot_style()
        
        # Get number of variants for test windows
        num_variants = window_stats['variants_per_window'][len(window_stats['variants_per_window'])//2:]
        
        # Ensure we use the same number of windows for both metrics and variants
        n_windows = min(len(num_variants), len(test_metrics_list))
        num_variants = num_variants[:n_windows]
        ...
        # Create subplots
        plt.subplot(3, 1, 1)
        plt.scatter(num_variants, macro_f1, c='blue', label='Macro F1', alpha=0.7, s=150)  # Increased marker size
        plt.scatter(num_variants, weighted_f1, c='red', label='Weighted F1', alpha=0.7, s=150)
        plt.title('F1 Scores vs Number of Variants', pad=20)
        plt.xlabel('Number of Variants')
        plt.ylabel('F1 Score')
        plt.legend(loc='upper right', frameon=True, fancybox=True, shadow=True, prop={'size': 16, 'weight': 'bold'})
        plt.grid(True, alpha=0.3)
        
        plt.subplot(3, 1, 2)
        plt.scatter(num_variants, macro_precision, c='blue', label='Macro Precision', alpha=0.7, s=150)
        plt.scatter(num_variants, weighted_precision, c='red', label='Weighted Precision', alpha=0.7, s=150)
        plt.title('Precision Scores vs Number of Variants', pad=20)
        plt.xlabel('Number of Variants')
        ...
        plt.subplot(3, 1, 3)
        plt.scatter(num_variants, macro_recall, c='blue', label='Macro Recall', alpha=0.7, s=150)
        plt.scatter(num_variants, weighted_recall, c='red', label='Weighted Recall', alpha=0.7, s=150)
        plt.title('Recall Scores vs Number of Variants', pad=20)
        plt.xlabel('Number of Variants')
        plt.ylabel('Recall')
        plt.legend(loc='upper right', frameon=True, fancybox=True, shadow=True, prop={'size': 16, 'weight': 'bold'})
        plt.grid(True, alpha=0.3)
        ...
        plt.tight_layout()
        plt.savefig(os.path.join(output_dir, 'metrics_vs_variants.png'))
        plt.close()
        
    except Exception as e:
        print(f"\nError in plot_metrics_vs_variants: {str(e)}")
        print(f"Number of windows in num_variants: {len(window_stats['variants_per_window'])//2}")
        ...
        plt.text(0.5, 0.5, f"Error plotting metrics: {str(e)}", 
                ha='center', va='center', transform=plt.gca().transAxes, fontsize=18, weight='bold')
        plt.savefig(os.path.join(output_dir, 'metrics_vs_variants.png'))
        plt.close()

def plot_metrics_vs_missing(test_metrics, window_stats, output_dir):
    ...
    try:
        ...
        n_windows = min(len(missing_percentages), len(test_metrics))
        missing_percentages = missing_percentages[:n_windows]
        
        # Extract metrics for each window
        macro_f1 = [m['macro_f1'] for m in test_metrics[:n_windows]]
        weighted_f1 = [m['weighted_f1'] for m in test_metrics[:n_windows]]
        macro_precision = [m['macro_precision'] for m in test_metrics[:n_windows]]
        weighted_precision = [m['weighted_precision'] for m in test_metrics[:n_windows]]
        macro_recall = [m['macro_recall'] for m in test_metrics[:n_windows]]
        weighted_recall = [m['weighted_recall'] for m in test_metrics[:n_windows]]
        
        plt.figure(figsize=(15, 12))
        
        # Create subplots
        plt.subplot(3, 1, 1)
        plt.scatter(missing_percentages, macro_f1, c='blue', label='Macro F1', alpha=0.7, s=100)
        plt.scatter(missing_percentages, weighted_f1, c='red', label='Weighted F1', alpha=0.7, s=100)
        plt.title('F1 Scores vs Missing Genotype Percentage', pad=20)
        ...
        plt.subplot(3, 1, 2)
        plt.scatter(missing_percentages, macro_precision, c='blue', label='Macro Precision', alpha=0.7, s=100)
        plt.scatter(missing_percentages, weighted_precision, c='red', label='Weighted Precision', alpha=0.7, s=100)
        plt.title('Precision Scores vs Missing Genotype Percentage', pad=20)
        plt.xlabel('Missing Genotype Percentage')
        plt.ylabel('Precision')
        plt.legend(loc='upper right', frameon=True, fancybox=True, shadow=True)
        plt.grid(True, alpha=0.3)
        ...
        plt.subplot(3, 1, 3)
        plt.scatter(missing_percentages, macro_recall, c='blue', label='Macro Recall', alpha=0.7, s=100)
        plt.scatter(missing_percentages, weighted_recall, c='red', label='Weighted Recall', alpha=0.7, s=100)
        plt.title('Recall Scores vs Missing Genotype Percentage', pad=20)
        plt.xlabel('Missing Genotype Percentage')
        plt.ylabel('Recall')
        plt.legend(loc='upper right', frameon=True, fancybox=True, shadow=True)
        plt.grid(True, alpha=0.3)
        ...
        print(f"Number of windows in test_metrics: {len(test_metrics)}")
        # Create an empty plot with error message
        plt.figure(figsize=(15, 10))
        plt.text(0.5, 0.5, f"Error plotting metrics: {str(e)}", 
                ha='center', va='center', transform=plt.gca().transAxes, fontsize=16, weight='bold')
        plt.savefig(os.path.join(output_dir, 'metrics_vs_missing.png'))
        plt.close()
...
def plot_test_metrics(test_metrics, output_dir):
    ...
    # Customize plot
    plt.title('Test Metrics Summary', pad=20, weight='bold')
...
def plot_confusion_matrix(confusion_matrix, output_dir):
    ...
    sns.heatmap(confusion_matrix, annot=True, fmt='d', cmap='Blues',
                xticklabels=['0/0', '0/1', '1/1'],
                yticklabels=['0/0', '0/1', '1/1'],
                annot_kws={'size': 18, 'weight': 'bold'})  # Increased annotation size
    plt.title('Confusion Matrix', pad=20, weight='bold')
...
def plot_genotype_distribution(genotype_counts, output_dir):
    ...
    bars = plt.bar(genotypes, counts, color=['#2ecc71', '#3498db', '#e74c3c'])
    
    plt.title('Distribution of Genotypes', pad=20, weight='bold')
    plt.xlabel('Genotype', weight='bold')
...
def plot_resource_usage(resource_history, output_dir):
    ...
    plt.subplot(1, 2, 1)
    plt.plot(resource_history['gpu_memory'], label='GPU Memory (MB)', linewidth=2)
    plt.title('GPU Memory Usage', pad=20, weight='bold')
    ...
    # Plot CPU usage
    plt.subplot(1, 2, 2)
    plt.plot(resource_history['cpu_percent'], label='CPU Usage (%)', linewidth=2)
    plt.title('CPU Usage', pad=20, weight='bold')
    plt.xlabel('Time Step', weight='bold')
    plt.ylabel('Usage (%)', weight='bold')
    plt.legend(loc='upper right', frameon=True, fancybox=True, shadow=True)
    plt.grid(True, alpha=0.3)
...
def plot_window_statistics(window_stats, output_dir):
    ...
    plt.subplot(1, 2, 1)
    plt.hist(window_stats['missing_per_window'], bins=50, color='#3498db', alpha=0.7)
    plt.title('Distribution of Missing Genotypes per Window', pad=20, weight='bold')
    plt.xlabel('Number of Missing Genotypes', weight='bold')
    plt.ylabel('Frequency', weight='bold')
    plt.grid(True, alpha=0.3)
    
    # Plot missing percentage per window
    plt.subplot(1, 2, 2)
    plt.hist(window_stats['missing_percentage'], bins=50, color='#e74c3c', alpha=0.7)
    plt.title('Distribution of Missing Percentage per Window', pad=20, weight='bold')
    plt.xlabel('Missing Percentage', weight='bold')
    plt.ylabel('Frequency', weight='bold')
    plt.grid(True, alpha=0.3)
...
def main():
    ...
    try:
        ...
        # Initialize lists to store all predictions and true labels for global metrics
        all_preds = []
        all_true = []
        
        test_pbar = tqdm(test_windows, desc='Test Phase')
        for window in test_pbar:
            features, positions, labels, missing_mask = extract_features(window[3])
            features = features.to(device)
            positions = positions.to(device)
            labels = labels.to(device)
            ...
            with torch.no_grad():
                ...
                predictions = torch.argmax(outputs, dim=-1)
                # Calculate loss
                loss = F.cross_entropy(outputs.view(-1, 3), labels.view(-1), ignore_index=-1).item()
            
            # Calculate per-window metrics
            metrics = calculate_metrics(predictions.view(-1), labels.view(-1), logger)
            metrics['loss'] = loss  # Add loss to metrics
            test_metrics.append(metrics)
            
            # Collect predictions and true labels for global metrics
            valid_mask = ~missing_mask
            all_preds.extend(predictions[valid_mask].cpu().numpy().flatten())
            all_true.extend(labels[valid_mask].cpu().numpy().flatten())
        ...
        global_metrics = {
            ...
            'macro_precision': precision_score(all_true, all_preds, average='macro'),
            'weighted_precision': precision_score(all_true, all_preds, average='weighted'),
            'macro_recall': recall_score(all_true, all_preds, average='macro'),
            'weighted_recall': recall_score(all_true, all_preds, average='weighted'),
        ...
        logger.info(f"Total test samples: {len(all_preds):,}")
        for k, v in global_metrics.items():
            if k != 'confusion_matrix':
                logger.info(f"  {k}: {v:.4f}")
            else:
        ...
        plot_test_metrics(global_metrics, args.output_dir)
        ...
        plot_metrics_vs_variants(test_metrics, window_stats, args.output_dir)
        plot_confusion_matrix(global_metrics['confusion_matrix'], args.output_dir)
